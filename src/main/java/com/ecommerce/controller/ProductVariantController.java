...
public class ProductVariantController {
  ...
  public ResponseEntity<ApiResponse<List<ProductVariant>>> createProductVariants(
    ...
    try {
      ...
      if (sessionId != null) {
        productVariants =
            variantService.batchUpsertVariantsInSession(
                sessionId, dto.getProductVariants(), productId);
      } else {
      ...
      return ResponseEntity.ok(
          new ApiResponse<>(
              true, productVariants, "Variants inserted successfully", null, LocalDateTime.now()));
    } catch (Exception e) {
      log.error("Error generating variants: {}", e.getMessage(), e);
  ...
  public ResponseEntity<ApiResponse<List<Map<String, Object>>>> generateVariants(
    ...
    try {
      ...
      log.info("Received data - ProductID: {}, DTO: {}", productId, dto);
      List<Map<String, Object>> variants = variantService.generateVariants(dto);
      return ResponseEntity.ok(
          new ApiResponse<>(
              true, variants, "Variants generated successfully", null, LocalDateTime.now()));
    } catch (Exception e) {
      log.error("Error while generating variants: {}", e.getMessage(), e);
      return ResponseEntity.badRequest()
          .body(new ApiResponse<>(false, null, null, e.getMessage(), LocalDateTime.now()));
    }
  }
